...
**Backend:** http://194.58.126.39:5001  
**Frontend:** http://194.58.126.39:4173
...
- [6. Тестирование Frontend UI](#6-тестирование-frontend-ui)
- [7. Проверка контейнеров на сервере](#7-проверка-контейнеров-на-сервере)
- [8. Сводная таблица результатов](#8-сводная-таблица-результатов)
- [9. Выявленные проблемы и решения](#9-выявленные-проблемы-и-решения)
...
## 6. Тестирование Frontend UI

### 6.1 Веб-интерфейс

**URL:** http://194.58.126.39:4173

**Функциональность:**
- ✅ Отображение общей информации о сервисе (message, endpoints)
- ✅ Отображение health (status: "ok", details)
- ✅ Отображение env (HOSTNAME, PORT)
- ✅ Калькулятор с операциями умножения и деления
- ✅ Результат вычислений отображается в JSON

### 6.2 Тест калькулятора
...
## 7. Проверка контейнеров на сервере

**Команда:**
```bash
ssh -i ~/.ssh/deploy_key deploy@194.58.126.39 "docker ps"
```

**Результат:**
```
CONTAINER ID   IMAGE                                        STATUS                    PORTS                    NAMES
11e2372005b5   ghcr.io/avkorolyov/actions:latest            Up (healthy)              0.0.0.0:5001->5001/tcp   flask-backend
23a39cdf4a56   ghcr.io/avkorolyov/actions-frontend:latest   Up                        0.0.0.0:4173->4173/tcp   flask-frontend
```

**Статус:** ✅ Оба контейнера работают

**Скриншоты:**

**Backend API (/) — главная страница с описанием эндпоинтов:**

![Backend API](../screenshots/Скрин%20Backend%20API.png)

**Frontend UI — веб-интерфейс с калькулятором (4 × 5 = 20):**

![Frontend UI](../screenshots/Скрин%20Frontend%20UI.png)

**GitHub Actions — успешный workflow с двумя jobs:**
...
|-----------|------|-----------|
| GitHub Actions | Сборка backend Docker-образа | ✅ Passed |
| GitHub Actions | Сборка frontend Docker-образа | ✅ Passed |
| GitHub Actions | Push в ghcr.io | ✅ Passed |
| GitHub Actions | Деплой через SSH | ✅ Passed |
| Backend API | GET / | ✅ Passed |
| Backend API | GET /health | ✅ Passed |
| Backend API | GET /info | ✅ Passed |
| Backend API | GET /env | ✅ Passed |
| Backend API | GET /multiply/3/4 | ✅ Passed |
| Backend API | GET /divide/10/2 | ✅ Passed |
| Frontend | Веб-интерфейс | ✅ Passed |
| Frontend | Калькулятор (4×5=20) | ✅ Passed |
| Docker | flask-backend (healthy) | ✅ Passed |
| Docker | flask-frontend | ✅ Passed |
...
1. ✅ Автоматически собираются Docker-образы (backend и frontend)
2. ✅ Образы публикуются в GitHub Container Registry
3. ✅ Приложения деплоятся на сервер через SSH
4. ✅ Backend API доступен по адресу http://194.58.126.39:5001/
5. ✅ Frontend UI доступен по адресу http://194.58.126.39:4173/
